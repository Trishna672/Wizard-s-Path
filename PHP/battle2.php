...
$enemy = [
    ...
    "level" => 7,
    "max_hp" => 80, // buffed so she doesn’t die in 1-2 hits
    "attack" => 14,
    "defense" => 6,
    "sprite" => "assets/bellatrix.png",
    "background" => "url('assets/battle_bg2.png')"
];

if (!isset($_SESSION['enemy_hp']) || $_SESSION['current_enemy'] !== 'battle2') {
    $_SESSION['enemy_hp'] = $enemy['max_hp'];
    $_SESSION['current_enemy'] = 'battle2';
    $_SESSION['submenu'] = 'main'; // track submenus
}

$player = &$_SESSION['player'];
if (!isset($_SESSION['player_max_hp'])) { $_SESSION['player_max_hp'] = $player['hp']; }
$message = "";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    elseif ($_SESSION['submenu'] === 'attack') {
        if (in_array($action, ["Expelliarmus","Stupefy","Sectumsempra"])) {
            ...
            if ($action === "Expelliarmus") $damage = rand($base+2, $base+6);
            if ($action === "Stupefy") $damage = rand($base+4, $base+10);
            if ($action === "Sectumsempra") $damage = rand($base+7, $base+14);
    ...
    // Run away
    elseif ($action === "run") {
        header("Location: outcome.php?outcome=lose&stage=2&reason=run");
        exit;
    }
    ...
    if ($_SESSION['enemy_hp'] <= 0) {
        ...
        header("Location: outcome.php?outcome=win&stage=2&next=battle3.php");
        exit;
    }
    if ($player['hp'] <= 0) {
        header("Location: outcome.php?outcome=lose&stage=2");
        exit;
    }
...
<head>
    <meta charset="UTF-8">
    <title>Battle 2 — <?= htmlspecialchars($enemy['name']) ?></title>
...
<div class="battle-container">
    ...
    <div id="game-container">
        <img id="character" src="<?= htmlspecialchars($_SESSION['player']['sprites']['battle'] ?? $_SESSION['player']['sprites']['display'] ?? 'assets/player_battle.png') ?>" alt="Player">
        <img id="enemy" src="<?= htmlspecialchars($enemy['sprite']) ?>" alt="Enemy">
    </div>

    <!-- Battle Log -->
    <div class="battle-log"><?= $message ?: "{$enemy['name']} cackles maniacally as the duel begins!"; ?></div>
